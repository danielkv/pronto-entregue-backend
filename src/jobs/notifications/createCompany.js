...
export async function createCompany({ data: { companyId } }) {
        ...
        include: [
            {
                ...
                where: where(
                    literal(
                        `(SELECT COUNT(id) FROM delivery_areas WHERE companyId = '${companyId}' AND ST_Distance_Sphere(\`addresses\`.\`location\`, center) <= radius)`,
                    ),
                    '>',
                    0,
                ),
    ...
    if (!users.length)
        throw new Error('Nenhum usuário para enviar notificaçãoes');

    // find tokens
    const pushTokenMetas = await UserMeta.findAll({
        where: {
            key: 'notification_tokens',
            userId: users.map((user) => user.get('id')),
        },
    });
    ...
    const notificationData = {
        ...
        priority: 'high',
        data: {
            redirect: new RedirectService('company', {
                ...comanyData,
            }),
