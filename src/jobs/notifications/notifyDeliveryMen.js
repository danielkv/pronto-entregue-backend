...
export async function notifyDeliveryMen(job) {
    ...
    NotificationController.sendDevice(tokens, {
        ...
        priority: 'high',
        data: {
            redirect: new RedirectService('deliveries'),
    ...
    if (orderId && repeat && repeat.count === repeat.limit) {
        ...
        const notificationMasterData = {
            ...
            body: `Estabelecimento ${company.get(
                'displayName',
            )} tem um pedido #${orderId} aguardando para retirada, mas nenhum entregador aceitou após ${
                repeat.count
            } notificações`,
            data: {
        ...
        const masterDesktopTokens = await UserController.getTokensByRole(
            'master',
            DESKTOP_TOKEN_META,
        );
        // send notifications
        NotificationController.sendDesktop(
            masterDesktopTokens,
            notificationMasterData,
        );

        // get master device tokens
        const masterDeviceTokens = await UserController.getTokensByRole(
            'master',
            DEVICE_TOKEN_META,
        );
        // send notifications
