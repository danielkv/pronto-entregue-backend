...
export default async function createDefaultOrder({ order, company }) {
    ...
    const [limitModel, intervalModel] = await configLoader.loadMany([
        ORDER_NOTIFICATION_LIMIT,
        ORDER_NOTIFICATION_INTERVAL,
    ]);

    const limit = deserializeConfig(
        limitModel.get('value'),
        limitModel.get('type'),
    );
    const interval = deserializeConfig(
        intervalModel.get('value'),
    ...
    JobQueue.notifications.add(`createOrder.${orderId}`, data, {
        repeat: { limit, every: interval, count: 0 },
    });
}
