...
export default async function createScheduledOrder ({ order, company }) {
	...
	const companyCreatedOrderMessage = {
		title: `${company.get('displayName')} recebeu um novo pedido encomendado`,
		body: `O pedido #${orderId} foi agendado para ${moment(scheduledTo).calendar()}.`
	}
		
	JobQueue.notifications.add(`simpleNotification.scheduledOrder.company.${orderId}`, {
	...
	JobQueue.notifications.add(`simpleNotification.scheduledOrder.user.${orderId}`, {
