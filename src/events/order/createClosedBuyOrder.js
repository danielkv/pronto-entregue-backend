...
export default async function createClosedBuyOrder({ order, company }) {
	...
	JobQueue.notifications.add(`simpleNotification.orderBuyClosed.${orderId}.company`, {
	...
	const { nextOpen } = await DB.companyMeta.findOne({
		attributes: [[Sequelize.fn('COMPANY_NEXT_OPEN_DATE', Sequelize.col('metas.value'), Sequelize.fn('NOW')), 'nextOpen']],
		where: { key: 'businessHours', companyId }
	})
	if (!nextOpen) return;

	const jobDelay = moment(nextOpen).subtract(15, 'minutes').diff(moment(), 'milliseconds');
	...
	const delayedNotificationCompanyData = {
		...
		body: `Este é apenas um lembrete, seu estabelecimento abre ${moment(nextOpen).calendar()} e seus clientes já estarão esperando esses pedidos.`,
		data: {
			action: 'orderCreated',
			variant: 'warning',
			orderId: _.toString(orderId),
