...
class CompanyControl {
    constructor() {
                ...
                let query = {
                    attributes: {
                        include: [
                            ...
                            [
                                Sequelize.fn(
                                    'COMPANY_ALLOW_BUY_CLOSED_BY_ID',
        ...
        this.metaLoader = new DataLoader(
            async (values) => {
    ...
    async getDeliveryType(companyId) {
        const meta = await CompanyMeta.findOne({
            where: { companyId, key: DELIVERY_TYPE_META },
        });
        ...
        let query = {
            attributes: {
                include: [
                    ...
                    [
                        Sequelize.fn('COMPANY_NEXT_CLOSE_DATE', Sequelize.col('metas.value')),
                        'nextClose',
                    ],
                    [
                        Sequelize.fn('COMPANY_ALLOW_BUY_CLOSED_BY_ID', Sequelize.col('company.id')),
                        'allowBuyClosed',
                    ],
                ],
            },
    ...
    getCompanies(where, location, pagination) {
        let query = {
            ...
            subQuery: false,
            order: [
                [Sequelize.col('isOpen'), 'DESC'],
                [Sequelize.col('allowBuyClosed'), 'DESC'],
            ],
        };
    ...
    includeQueryLocation(location, query = {}, filterLocation = true) {
        ...
        if (filterLocation) {
            ...
            if (query.where) query.where = [query.where, addToQuery];
            else query.where = addToQuery;
        }

        const attributesInclude =
            query.attributes && query.attributes.include ? query.attributes.include : [];
        query.attributes = {
            include: [
                ...attributesInclude,
                [
    ...
    async setConfig(companyId, key, value, type) {
        const meta = await DB.companyMeta.findOne({
            where: { companyId, key },
        });
        if (!meta) {
            const valueSave = serealizeConfig(value, type);
            const created = await DB.companyMeta.create({
                key,
                companyId,
                value: valueSave,
                type,
    ...
    includeArea(userPoint, model, type) {
        return {
            ...
            where: [
                Sequelize.where(
                    Sequelize.fn('ST_Distance_Sphere', userPoint, Sequelize.col(`${type}.center`)),
                    '<=',
                    Sequelize.col(`${type}.radius`),
                ),
